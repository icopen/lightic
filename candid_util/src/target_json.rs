use candid::{TypeEnv, types::{Type, Field}, parser::types::FuncMode};
...
pub fn print_type(ty: &Type) -> String {
    ...
    match ty {
        ...
        Type::Func(f) => {
            str.push_str(&format!("{{ \"Func\": {{ \"args\": {}, \"rets\": {}, \"modes\": {} }} }}", print_array(&f.args), print_array(&f.rets), print_modes(&f.modes)));
        },
        Type::Empty |
        Type::Reserved |
        Type::Unknown
...
pub fn compile(env: &TypeEnv, actor: &Option<Type>) -> String {
    ...
    match actor {
        None => {},
        Some(actor) => { result.push_str(&print_type(actor)); }
    }
