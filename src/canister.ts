...
export class Canister {
  ...
  async init (initArgs: any, candidSpec?: string): Promise<void> {
    ...
    if (this.idl.init_args !== undefined && this.idl.init_args !== null && this.idl.init_args.length > 0) {
      ...
      // Initialize canister
      const msg = Message.init(this.id, args)
    ...
    const importObject = {
      ic0: {
        ...
        msg_reject_code: (...args) =>
          this.not_implemented.apply(self, ['msg_reject_code', ...args]),
        msg_reject_msg_size: (...args) =>
          this.not_implemented.apply(self, ['msg_reject_msg_size', ...args]),
        msg_reject_msg_copy: (...args) =>
          this.not_implemented.apply(self, ['msg_reject_msg_copy', ...args]),

        msg_reply_data_append: (...args) =>
          this.msg_reply_data_append.apply(self, args),
        msg_reply: (...args) => this.msg_reply.apply(self, args),
        msg_reject: (...args) =>
          this.not_implemented.apply(self, ['msg_reject', ...args]),
